...
export class ZrenderHelperService {
  ...
  constructor(
    ...
    storeSrv.getType().subscribe(e => {
      if (e === 'stick') {
        this.isStick = true;
        this.screenOffsetX = 172;
        this.screenOffsetY = 124;
        this.eRatio = 0.5;
      } else if (e === 'core') {
        this.isStick = false;
  ...
  createToolBarSvg(toolBar: ToolBar) {
    toolBar.toolList.forEach((toolItem, index) => {
      ...
      g.position = [index * 48 * this.scaleRatio, 0];
  ...
  createUnitArea() {
    ...
    const g = new zrender.Group({});
    g.position = [50 * this.scaleRatio, 520 * this.scaleRatio];
    ...
    text.position = [50 * this.scaleRatio, 520 * this.scaleRatio];
